...
export class TenantPaymentStatusService {
  static async getAll(tenantId: number, filters?: { is_active?: boolean }): Promise<any[]> {
    let query = 'SELECT * FROM tenant_payment_statuses WHERE tenant_id = ?';
    ...
    for (const row of rows) {
      const [translations] = await pool.query<RowDataPacket[]>(
        `SELECT t.*, l.code as language_code, l.name as language_name
         FROM tenant_payment_status_translations t
         JOIN languages l ON t.language_id = l.id
         WHERE t.tenant_payment_status_id = ?
  ...
  static async getById(tenantId: number, id: number): Promise<any | null> {
    const [rows] = await pool.query<RowDataPacket[]>(
      'SELECT * FROM tenant_payment_statuses WHERE id = ? AND tenant_id = ?', [id, tenantId]
    );
  ...
  static async create(tenantId: number, data: {
    ...
    try {
      ...
      const [existing] = await conn.query<RowDataPacket[]>(
        'SELECT id FROM tenant_payment_statuses WHERE tenant_id = ? AND code = ?', [tenantId, data.code]
      );
      if (existing.length > 0) throw { status: 409, message: 'Payment status with this code already exists' };
      const [result] = await conn.query<ResultSetHeader>(
        'INSERT INTO tenant_payment_statuses (tenant_id, master_payment_status_id, code, color, sort_order, is_active) VALUES (?, ?, ?, ?, ?, ?)',
        [tenantId, data.master_payment_status_id || null, data.code, data.color || null,
      ...
      if (data.translations?.length) {
        for (const t of data.translations) {
          await conn.query(
            'INSERT INTO tenant_payment_status_translations (tenant_payment_status_id, language_id, name) VALUES (?, ?, ?)',
  ...
  static async update(tenantId: number, id: number, data: {
    ...
    try {
      await conn.beginTransaction();
      const [existing] = await conn.query<RowDataPacket[]>(
        'SELECT id FROM tenant_payment_statuses WHERE id = ? AND tenant_id = ?', [id, tenantId]
      );
      if (existing.length === 0) throw { status: 404, message: 'Payment status not found' };
      if (data.code) {
        const [dup] = await conn.query<RowDataPacket[]>(
          'SELECT id FROM tenant_payment_statuses WHERE tenant_id = ? AND code = ? AND id != ?', [tenantId, data.code, id]
        );
      ...
      if (fields.length > 0) {
        ...
        await conn.query(`UPDATE tenant_payment_statuses SET ${fields.join(', ')} WHERE id = ? AND tenant_id = ?`, values);
      }
      if (data.translations) {
        await conn.query('DELETE FROM tenant_payment_status_translations WHERE tenant_payment_status_id = ?', [id]);
        for (const t of data.translations) {
          await conn.query(
            'INSERT INTO tenant_payment_status_translations (tenant_payment_status_id, language_id, name) VALUES (?, ?, ?)',
  ...
  static async delete(tenantId: number, id: number): Promise<boolean> {
    const [result] = await pool.query<ResultSetHeader>(
      'DELETE FROM tenant_payment_statuses WHERE id = ? AND tenant_id = ?', [id, tenantId]
    );
  ...
  static async getAvailableMaster(tenantId: number): Promise<any[]> {
    const [rows] = await pool.query<RowDataPacket[]>(`
      ...
      FROM master_payment_statuses m
      LEFT JOIN tenant_payment_statuses t ON t.master_payment_status_id = m.id AND t.tenant_id = ?
      WHERE m.is_active = 1
    ...
    try {
      ...
      for (const masterId of masterIds) {
        const [already] = await conn.query<RowDataPacket[]>(
          'SELECT id FROM tenant_payment_statuses WHERE tenant_id = ? AND master_payment_status_id = ?', [tenantId, masterId]
        );
        if (already.length > 0) continue;
        const [masters] = await conn.query<RowDataPacket[]>('SELECT * FROM master_payment_statuses WHERE id = ?', [masterId]);
        if (masters.length === 0) continue;
        const master = masters[0];
        const [result] = await conn.query<ResultSetHeader>(
          'INSERT INTO tenant_payment_statuses (tenant_id, master_payment_status_id, code, color, sort_order, is_active) VALUES (?, ?, ?, ?, ?, 1)',
        ...
        const [masterTranslations] = await conn.query<RowDataPacket[]>(
          'SELECT * FROM master_payment_status_translations WHERE master_payment_status_id = ?', [masterId]
        );
