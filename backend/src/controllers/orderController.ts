import { Response } from 'express';
import { TenantAuthRequest } from '../middleware/tenantAuth.js';
import { OrderService } from '../services/orderService.js';

export class OrderController {
  static async getAll(req: TenantAuthRequest, res: Response): Promise<void> {
    try {
      if (!req.tenant) { res.status(400).json({ error: 'Tenant context required' }); return; }
      const tenantId = Number(req.tenant.id);
      const filters = {
        store_id: req.query.store_id ? Number(req.query.store_id) : undefined,
        order_status: req.query.order_status ? String(req.query.order_status) : undefined,
        tenant_customer_id: req.query.tenant_customer_id ? Number(req.query.tenant_customer_id) : undefined,
        tenant_waiter_id: req.query.tenant_waiter_id ? Number(req.query.tenant_waiter_id) : undefined,
        table_id: req.query.table_id ? Number(req.query.table_id) : undefined,
        from_date: req.query.from_date ? String(req.query.from_date) : undefined,
        to_date: req.query.to_date ? String(req.query.to_date) : undefined,
        limit: req.query.limit ? Number(req.query.limit) : undefined,
        offset: req.query.offset ? Number(req.query.offset) : undefined,
      };
      const items = await OrderService.getAll(tenantId, filters);
      res.json({ data: items });
    } catch (error: any) {
      console.error('[OrderController] getAll error:', error);
      res.status(500).json({ error: 'Failed to fetch orders' });
    }
  }

  static async getById(req: TenantAuthRequest, res: Response): Promise<void> {
    try {
      if (!req.tenant) { res.status(400).json({ error: 'Tenant context required' }); return; }
      const id = parseInt(req.params.id);
      if (isNaN(id)) { res.status(400).json({ error: 'Invalid ID' }); return; }
      const item = await OrderService.getById(Number(req.tenant.id), id);
      if (!item) { res.status(404).json({ error: 'Order not found' }); return; }
      res.json({ data: item });
    } catch (error: any) {
      console.error('[OrderController] getById error:', error);
      res.status(500).json({ error: 'Failed to fetch order' });
    }
  }

  static async create(req: TenantAuthRequest, res: Response): Promise<void> {
    try {
      if (!req.tenant) { res.status(400).json({ error: 'Tenant context required' }); return; }
      const tenantId = Number(req.tenant.id);
      const id = await OrderService.create(tenantId, req.body);
      const item = await OrderService.getById(tenantId, id);
      res.status(201).json({ data: item, message: 'Order created successfully' });
    } catch (error: any) {
      if (error.status === 400) { res.status(400).json({ error: error.message }); return; }
      console.error('[OrderController] create error:', error);
      res.status(500).json({ error: 'Failed to create order' });
    }
  }

  static async update(req: TenantAuthRequest, res: Response): Promise<void> {
    try {
      if (!req.tenant) { res.status(400).json({ error: 'Tenant context required' }); return; }
      const tenantId = Number(req.tenant.id);
      const id = parseInt(req.params.id);
      if (isNaN(id)) { res.status(400).json({ error: 'Invalid ID' }); return; }
      await OrderService.update(tenantId, id, req.body);
      const item = await OrderService.getById(tenantId, id);
      res.json({ data: item, message: 'Order updated successfully' });
    } catch (error: any) {
      if (error.status === 404) { res.status(404).json({ error: error.message }); return; }
      if (error.status === 400) { res.status(400).json({ error: error.message }); return; }
      console.error('[OrderController] update error:', error);
      res.status(500).json({ error: 'Failed to update order' });
    }
  }

  static async delete(req: TenantAuthRequest, res: Response): Promise<void> {
    try {
      if (!req.tenant) { res.status(400).json({ error: 'Tenant context required' }); return; }
      const id = parseInt(req.params.id);
      if (isNaN(id)) { res.status(400).json({ error: 'Invalid ID' }); return; }
      const deleted = await OrderService.delete(Number(req.tenant.id), id);
      if (!deleted) { res.status(404).json({ error: 'Order not found' }); return; }
      res.json({ message: 'Order deleted successfully' });
    } catch (error: any) {
      console.error('[OrderController] delete error:', error);
      res.status(500).json({ error: 'Failed to delete order' });
    }
  }
}
